...
class MegadriverApp(ttk.Frame):
    ...
    def initUI(self):
        ...
        midiPortCombo.grid(row=0, column=1, padx=10, pady=10)
        ...
        presetSelCombo.grid(row=0, column=3, padx=10, pady=10)
        ...
        lfoEnCheckButton.grid(row=1, column=1, padx=10, pady=10)
        ...
        lfoRateScale.grid(row=1, column=2, padx=10, pady=10)
        ...
        for voiceId in range(YM2612.YM_MAX_VOICES):
            ...
            voiceFrame.grid(row=3, column=0, rowspan=2, columnspan=8, sticky=tk.W+tk.E+tk.N+tk.S, padx=10, pady=10)
            tabVociceFrameList.append(voiceFrame)
            self.tabVoiceList.add(voiceFrame, text = 'Voice %d' % (voiceId))

        self.tabVoiceList.grid(row=2, column=0, rowspan=13, columnspan=8, sticky=tk.W+tk.E+tk.N+tk.S, padx=10, pady=10)
        ...
        for voiceId in range(YM2612.YM_MAX_VOICES):
            ...
            labelFrame.grid(row=0, column=0, rowspan=2, columnspan=8, sticky=tk.W+tk.E+tk.N+tk.S, padx=10, pady=10)
            self.frameVoiceParameter.append(labelFrame)

            # Feedback control
            voiceFeedbackLbl = ttk.Label(labelFrame, text='FEEDBACK')
            voiceFeedbackLbl.grid(row=0, column=0, padx=10, pady=10)
            ...
            voiceFeedbackScale.grid(row=0, column=1, padx=10, pady=10)

            # Algorithm control
            voiceAlgorithmLbl = ttk.Label(labelFrame, text='ALGORITHM')
            voiceAlgorithmLbl.grid(row=0, column=2, padx=10, pady=10)
            ...
            voiceAlgorithmScale.grid(row=0, column=3, padx=10, pady=10)

            # Output
            voiceAlgorithmLbl = ttk.Label(labelFrame, text='OUT')
            voiceAlgorithmLbl.grid(row=0, column=4, padx=10, pady=10)
            ...
            voiceAMSScale.grid(row=1, column=1, padx=10, pady=10)

            # Phase Modulation Sens
            voicePMSLbl = ttk.Label(labelFrame, text='PMS')
            voicePMSLbl.grid(row=1, column=2, padx=10, pady=10)
            ...
            voicePMSScale.grid(row=1, column=3, padx=10, pady=10)
            ...
            for operatorId in range(YM2612.YM_MAX_OPERATORS):
                ...
                operatorDetuneScale.grid(row=0, column=1, padx=10, pady=10)
                ...
                operatorMultipleScale.grid(row=1, column=1, padx=10, pady=10)
                ...
                operatorKeyScale.grid(row=2, column=1, padx=10, pady=10)
                ...
                operatorAmpModCheckBox.grid(row=3, column=1, padx=10, pady=10)
                ...
                operatorSSEGCombo.grid(row=4, column=1, padx=10, pady=10)
                ...
                operatorTotalLevelScale.grid(row=5, column=1, padx=10, pady=10)
                ...
                operatorAttackRateScale.grid(row=6, column=1, padx=10, pady=10)
                ...
                operatorSustainLevelScale.grid(row=7, column=1, padx=10, pady=10)
                ...
                operatorSustainRateScale.grid(row=8, column=1, padx=10, pady=10)
                ...
                operatorDecayRateScale.grid(row=9, column=1, padx=10, pady=10)
                ...
                operatorReleaseRateScale.grid(row=10, column=1, padx=10, pady=10)
    ...
    def loadPreset(self):
        ...
        if filename is not None:
            ...
            for voice in range(YM2612.YM_MAX_VOICES):
                self._loadVgiFile(filename, voice)
        if self.varLiveUpdate.get():
            self.sendCommand()


    def _readByte(self, fileHandler):
    ...
    def _loadVgiFile(self, vgiFilename, voiceId):
        with open(vgiFilename, 'rb') as byte_reader:
            byte_data = byte_reader.read()
            if len(byte_data) == 43:
                byte_reader.seek(0)
                # Read voice parameters
                self.varVoiceAlgorithm[voiceId].set(self._readByte(byte_reader))
                self.varVoiceFeedback[voiceId].set(self._readByte(byte_reader))
                reg = self._readByte(byte_reader)
                ...
                for operator_id in range(YM2612.YM_MAX_OPERATORS):
                    self.varOperatorMultiple[voiceId][operator_id].set(self._readByte(byte_reader))
                    self.varOperatorDetune[voiceId][operator_id].set(self._readByte(byte_reader))
                    self.varOperatorTotalLevel[voiceId][operator_id].set(self._readByte(byte_reader))
                    self.varOperatorKeyScale[voiceId][operator_id].set(self._readByte(byte_reader))
                    self.varOperatorAttackRate[voiceId][operator_id].set(self._readByte(byte_reader))
                    reg = self._readByte(byte_reader)
                    self.varOperatorAmpMod[voiceId][operator_id].set((reg >> 7) & 0x01)
                    self.varOperatorDecayRate[voiceId][operator_id].set((reg >> 0) & 0x1F)
                    self.varOperatorSustainRate[voiceId][operator_id].set(self._readByte(byte_reader))
                    self.varOperatorReleaseRate[voiceId][operator_id].set(self._readByte(byte_reader))
                    self.varOperatorSustainLevel[voiceId][operator_id].set(self._readByte(byte_reader))
                    reg = self._readByte(byte_reader)
    ...
    def _saveVgiFile(self, vgiFilename, voiceId):
        with open(vgiFilename, 'wb') as byte_writer:
            byte_writer.seek(0)
            # Write voice parameters
            byte_writer.write(self._writeByte(self.varVoiceAlgorithm[voiceId].get()))
            byte_writer.write(self._writeByte(self.varVoiceFeedback[voiceId].get()))
            ...
            byte_writer.write(self._writeByte(regVar))
            # Write operator parameters
            for operator_id in range(YM2612.YM_MAX_OPERATORS):
                byte_writer.write(self._writeByte(self.varOperatorMultiple[voiceId][operator_id].get()))
                byte_writer.write(self._writeByte(self.varOperatorDetune[voiceId][operator_id].get()))
                byte_writer.write(self._writeByte(self.varOperatorTotalLevel[voiceId][operator_id].get()))
                byte_writer.write(self._writeByte(self.varOperatorKeyScale[voiceId][operator_id].get()))
                byte_writer.write(self._writeByte(self.varOperatorAttackRate[voiceId][operator_id].get()))
                regVar = self.varOperatorAmpMod[voiceId][operator_id].get() << 7
                regVar |= self.varOperatorDecayRate[voiceId][operator_id].get()
                byte_writer.write(self._writeByte(regVar))
                byte_writer.write(self._writeByte(self.varOperatorSustainRate[voiceId][operator_id].get()))
                byte_writer.write(self._writeByte(self.varOperatorReleaseRate[voiceId][operator_id].get()))
                byte_writer.write(self._writeByte(self.varOperatorSustainLevel[voiceId][operator_id].get()))
                regVar = SSGEGValues.get(self.comboOperatorSSGEG[voiceId][operator_id].get())
                byte_writer.write(self._writeByte(regVar))
            self.log.info("Saved %d bytes" % byte_writer.tell())


    def _syncDriver(self):
    ...
    def testButton(self):
        ...
        if filename is not None:
            self.log.info(filename)
            self.FmChip.load_vgi_file(filename)
            # Show values registered
            self.FmChip.show_reg_values()
    ...
    def savePreset(self):
        ...
        if filename is not None:
            ...
            self._saveVgiFile(filename,voiceId)
